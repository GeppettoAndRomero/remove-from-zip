...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/remove-from-zip/sw.js, scope=/remove-from-zip/）。
    // scope がツールの全 locale ページ（/remove-from-zip/, /remove-from-zip/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/remove-from-zip/sw.js', {
