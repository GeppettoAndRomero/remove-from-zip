# remove-from-zip — stamped from heic-to-jpg, ENGINE + CONTENT still TODO

Frozen core and the slug surface are done. This tool currently builds as a HEIC clone
at /remove-from-zip/. Replace the engine layer before it is real:
...
- [ ] `src/i18n/{en,ja,zh,de,es}.ts` — 5 native content sets via the competitor-grounded
      ...
      Also fix the PWA display name in `public/remove-from-zip/manifest.json` ("HEIC to JPG").
...
Deploy (push-to-deploy, zero Cloudflare manual ops — handbook/DEPLOY-MODEL.md):
  1. create the GitHub repo + push (gh repo create GeppettoAndRomero/remove-from-zip --source=. --push).
  2. handbook/bin/deploy-onboard.sh remove-from-zip   # sets CF secrets on the repo (no dashboard).
  3. push to main → Actions builds → `wrangler pages deploy` auto-creates the Pages
     project `remove-from-zip` and deploys. The frozen-core .github/workflows/deploy.yml is already
